...
from util.estimation_postprocessing import retrieve_observation_residuals

from util.point_distributions import fibonacci_sphere, random_sphere
...
body_settings.get("MEX").ephemeris_settings = environment_setup.ephemeris.tabulated_from_existing(
...
random_sphere
# Create TW stations
for i, (station_latitude, station_longitude) in enumerate(fibonacci_sphere(50)):
    print(i, station_latitude, station_longitude)
...
tw_mex_links = []
for i in range(n_TW):
    one_way_tw_mex_link_ends = dict( )
    one_way_tw_mex_link_ends[observation.transmitter] = observation.body_reference_point_link_end_id("Mars", f"TW{i}")
    one_way_tw_mex_link_ends[observation.receiver] = observation.body_origin_link_end_id("MEX")
    one_way_tw_mex_link_definition = observation.LinkDefinition(one_way_tw_mex_link_ends)
    tw_mex_links.append(one_way_tw_mex_link_definition)
...
for i in range(n_TW):
    viability_settings = [observation.elevation_angle_viability(["Mars", f"TW{i}"], np.deg2rad(15))]
                        #   observation.body_occultation_viability([f"TW{i}", "MEX"], "Mars")]
    observation.add_viability_check_to_observable_for_link_ends(
        observation_simulation_settings,
        viability_settings,
        estimation_setup.observation.one_way_range_type,
        tw_mex_links[i])
...
doppler_tw0_residuals = retrieve_observation_residuals(final_residuals, mex_simulated_observations, observation.one_way_instantaneous_doppler_type)
range_tw0_residuals = retrieve_observation_residuals(final_residuals, mex_simulated_observations, observation.one_way_range_type)

doppler_observables_start, doppler_observables_count = mex_simulated_observations.observable_type_start_index_and_size[observation.one_way_instantaneous_doppler_type]
range_observables_start, range_observables_count = mex_simulated_observations.observable_type_start_index_and_size[observation.one_way_range_type]

ax1.scatter((observation_times[doppler_observables_start:(doppler_observables_start+doppler_observables_count)] - observation_times[0]) / (3600*24),
            final_residuals[doppler_observables_start:(doppler_observables_start+doppler_observables_count)], 
            color="blue")
ax1.scatter((observation_times[range_observables_start:(range_observables_start+range_observables_count)] - observation_times[0]) / (3600*24),
            final_residuals[range_observables_start:(range_observables_start+range_observables_count)], 
            color="red")
